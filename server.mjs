...
async function generateGraph(input, debugFile2flow = false) {
  ...
  if (suggestedFlowTitle) {
    ...
    if (defNode) {
      defNode.label = suggestedFlowTitle;
      if (!defNode.content?.abbrevTouched) {
        defNode.content = {
          ...(defNode.content || {}),
          abbrev: abbrevFromAgreementName(suggestedFlowTitle),
          abbrevTouched: false,
        };
      }
    }
...
function normalizeAiGraph(raw, input) {
  ...
  const nodes = raw.nodes.map((n, index) => {
    ...
    if (type === NodeType.DEFINITION) {
      const agreementLabel = String(n.label || n.content?.title || '');
      if (!String(content.abbrev || '').trim()) {
        content.abbrev = abbrevFromAgreementName(agreementLabel);
        content.abbrevTouched = Boolean(content.abbrevTouched);
      }
    }
    return {
      id: nodeId,
