...
function NodeView({ node, selected, onPointerDown, onUpdate, onDelete, onDuplicate, onStartConn, onFinishConn, onOpenMenu, hoverPort, setHoverPort, pendingConn, edges, readOnly, registerPort, onNodeMount }) {
  ...
  /* ── Definition node: agreement name, abbrev, definition body ── */
  if (node.type === 'definition') {
    const effectiveAbbrev = node.abbrevTouched
      ? (node.abbrev || '')
      : (node.abbrev || abbrevFromAgreementName(node.title) || '');
    return (
      <div
      ...
      >
        ...
        <div style={{ padding: '10px 12px 14px' }}>
          <input
            ...
            style={{ fontFamily: 'var(--font-headline)', fontSize: 17, fontWeight: 700, marginBottom: 6, letterSpacing: '-0.01em', display: 'block', width: '100%' }}
            value={node.title || ''}
            onChange={(e) => {
              const title = e.target.value;
              const patch = { title };
              if (!node.abbrevTouched) patch.abbrev = abbrevFromAgreementName(title);
              onUpdate(patch);
            }}
            onPointerDown={(e) => e.stopPropagation()}
          ...
          {readOnly ? (
            effectiveAbbrev ? (
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 600, color: 'var(--purple-700)', marginBottom: 8, letterSpacing: '0.04em' }}>
                {effectiveAbbrev.toUpperCase()}
              </div>
            ) : null
          ) : (
            <input
              ...
              value={effectiveAbbrev}
              onChange={(e) => onUpdate({
                abbrev: e.target.value.toUpperCase().slice(0, 4),
                abbrevTouched: true,
              })}
              onPointerDown={(e) => e.stopPropagation()}
          ...
          <textarea
            className="node-title-input"
            style={{ fontSize: 13.5, lineHeight: 1.65, color: 'var(--ink-700)', display: 'block', width: '100%', resize: 'none', minHeight: 176 }}
            value={node.body || ''}
    ...
    <aside className="rightpanel">
      ...
      <div className="rp-body" style={{ gap: 0, padding: 0 }}>
        ...
        <div style={{ padding: '18px 16px 16px' }}>
          ...
          {defNode ? (
            <>
              <div style={{ fontFamily: 'var(--font-headline)', fontSize: 14, fontWeight: 700, color: 'var(--ink-900)', marginBottom: 4 }}>{defNode.title || 'Agreement Name'}</div>
              {(defNode.abbrevTouched ? defNode.abbrev : (defNode.abbrev || abbrevFromAgreementName(defNode.title))) ? (
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 600, color: 'var(--purple-700)', marginBottom: 8 }}>
                  {(defNode.abbrevTouched ? (defNode.abbrev || '') : (defNode.abbrev || abbrevFromAgreementName(defNode.title) || '')).toUpperCase()}
                </div>
              ) : null}
